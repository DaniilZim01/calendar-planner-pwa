...
### Push-уведомления: расписание

- Из-за лимитов Vercel Hobby оставили только утреннее уведомление в 09:00 локального времени пользователя (вечернее отключено).
- Храним `timezone` и `tz_offset` в таблице `push_subscriptions`, чтобы вычислять локальное время на сервере без зависимости от часового пояса деплоя.
